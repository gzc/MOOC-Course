...
object Anagrams {
   ...
   */
  def wordOccurrences(w: Word): Occurrences = (w.toLowerCase.toList.groupBy((element: Char) => element) map {
  	case(x : Char, y : List[Char]) => (x, y.length)
  ...
  def combinations(occurrences: Occurrences): List[Occurrences] = {
      ...
      i <- 0 to count
      rest = occurrences.filter(y => y._1 > char)
  ...
  def subtract(x: Occurrences, y: Occurrences): Occurrences = {
      ...
      if(xTuple._1 == yTuple._1)
        ...
        xTuple
    }).filter(x => x._2 != 0).toSet.toList.sortWith( _._1 < _._1)
  }
