#include <iostream>
#include <string>
#include <vector>

using namespace std;

#define MAX_KEY_LENGTH 13

int convert(char ch)
{
    if(ch >= '0' && ch <= '9') return ch-'0';
    else return ch-'A'+10;
}


int main()
{
    string ciphertext = "";
    
    vector<unsigned char> cipher;
    
    int i = 0;
    while(i < ciphertext.size())
    {
        unsigned char ch = convert(ciphertext[i])*16 + convert(ciphertext[i+1]);
        cipher.push_back(ch);
        i += 2;
    }
    
    
    /*
    cout << ciphertext.size() << "    "  << cipher.size() << endl;
    cout << 1.0/256 << endl;
    for(int k = 1;k <= MAX_KEY_LENGTH; ++k)
    {
        int p[256] = {0};
        int sum(0);
        for(int j = 0;j < cipher.size();j = j+k)
        {
            sum++;
            p[cipher[j]]++;
        }
        
        double squareP(0);
        for(i = 0;i < 256;i++)
        {
            double tmp = p[i]*1.0/sum;
            squareP += tmp*tmp;
        }
        cout << k << "   "  << squareP << endl;
    }*/
    
    /*
    int len = 7;
    vector<int>backup;
    
    for(int ch = 0; ch < 256;ch++)
    {
        backup.push_back(ch);
        for(int j = 0;j < cipher.size();j = j+len)
        {
            int c = ch ^ cipher[j];
            if(c <= 31 || (c >= '0' && c <= '9') || c >= 127)
            {
                backup.pop_back();
                break;
            }
        }
    }
    
    cout << backup.size() << endl;
    
    
    for(int k = 0;k < backup.size(); k++)
    {
        int key = backup[k];
        
        vector<int> p(256, 0);
        int maxIndex(-1),max(-1);
        
        for(int j = 0;j < cipher.size();j = j+len)
        {
            int v = key ^ cipher[j];
            p[v]++;
            if(p[v] > max)
            {
                max = p[v];
                maxIndex = v;
            }
        }
        
        cout << key << "  " <<(char)maxIndex << "   "  << p['a']  << "  " << p['e'] << "  " << p['i'] << "  " << p['o'] << "  "
        <<p['x'] << "  " << p['q'] << "  " << p['j'] << "  " << p['z'] << endl;
        
    }*/
    
    unsigned char key[7] = {186, 31, 145, 178, 83, 205, 62};
    for(int k = 0;k < cipher.size();k++)
    {
        int index = k%7;
        unsigned char ch = cipher[k] ^ key[index];
        cout << (char)ch;
    }
    
}

//0x31 145 178 83 205 62 186